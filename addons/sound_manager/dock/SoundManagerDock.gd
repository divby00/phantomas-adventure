...
func populate_files_list(file_names: PoolStringArray) -> void:
	...
	for i in range(0, file_names.size()):
		...
		if file_extension == "wav" or file_extension == "ogg" or file_extension == "mp3" or file_extension == "opus":
			# Set the nodes
			...
			add_entry_button.connect(
				"pressed", self, "_on_add_entry_button_pressed", [dir_line.get_text() + file_line.get_text()]
			)
...
#
	...
	save_entry_button.connect(
		"pressed", self, "_on_save_entry_button_pressed", [save_entry_button.get_parent().get_index()]
	)
	remove_entry_button.connect(
		"pressed", self, "_on_remove_entry_button_pressed", [entry_container.name, key_input.text]
	)
