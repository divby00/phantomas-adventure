...
func get_sound_file_names_from_path_r(path: String) -> PoolStringArray:
	var directory: EditorFileSystemDirectory = get_editor_interface().get_resource_filesystem().get_filesystem_path(
		path
	)
	...
	if directory:
		for i in range(0, directory.get_file_count()):
			...
			if (
				...
				or file_name.get_extension() == "mp3"
				or file_name.get_extension() == "wav"
